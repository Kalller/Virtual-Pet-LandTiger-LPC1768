...
* Attention		 : None 
*******************************************************************************/
extern void wait_delay(int count)
{
	while(count--);
}
...
void PutReverseChar( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
		int16_t i, j;
		uint8_t buffer[16], tmp_char;
    GetASCIICode(buffer,ASCI);  /* 取字模数据 */
...
void GUI_ReverseText(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor)
{
    uint8_t TempChar;
    do
    {
        TempChar = *str++;  
        PutReverseChar( Xpos, Ypos, TempChar, Color, bkColor );    
        if( Xpos > 8 )
        {
            Xpos -= 8;
        } 
